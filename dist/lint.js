...
import { resolveGuides } from "./discovery.js";
...
function analyseFences(content        )                {
...
function findLinkTargets(content        , fencedLines             )               {
  const targets               = [];
  const lines = linesOf(content);
...
function relativePathFromTarget(rawTarget        )                     {
      ...
      diagnostics.push({
        ...
        message:
          code === "ENOENT" || code === "ENOTDIR"
            ...
            : `Cannot inspect relative Markdown target ${target.raw}: ${(error         ).message}`,
        file: guide.path,
        line: target.line,
        column: target.column,
        target: resolved,
...
function findScriptReferences(content        )                    {
  const references                    = [];
  const seen = new Set        ();

  const add = (manager        , script        , offset        )       => {
  ...
  while (true) {
    ...
    try {
      ...
      if ((error                         ).code !== "ENOENT") {
        return {
          path: packagePath,
          error: `Cannot read package.json: ${(error         ).message}`,
        };
      }
    }
...
function checkFence(guide           , analysis               )               {
