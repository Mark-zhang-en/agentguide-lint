...
import { ConfigurationError, resolveGuides } from "./discovery.ts";
import { formatOutput, summarize } from "./format.ts";
import { checkProject } from "./lint.ts";
...
export function parseArgs(args: string[]): CliOptions | "help" | "version" {
  ...
  for (let index = 1; index < args.length; ) {
    ...
    if (formatOption) {
      if (!(["text", "json", "sarif"] as string[]).includes(formatOption.value)) {
        throw new ConfigurationError(
          `Unknown output format: ${formatOption.value}`,
        );
      }
      format = formatOption.value as OutputFormat;
    ...
    const result =
      options.command === "check"
        ...
        : {
            ...
            diagnostics: [] as Awaited<ReturnType<typeof checkProject>>["diagnostics"],
          };
